...
class HttpKernelOldTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new HttpKernel($dispatcher, $this->getResolver('controller_func'));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new HttpKernel($dispatcher, $this->getResolver(array('Controller', 'staticcontroller')));
	...
	$resolver = $this->getMock('Minima\\Controller\\ControllerResolverInterface');
        $resolver->expects($this->any())
            ->method('resolve')
            ->will($this->returnValue(array($controller, array())));
