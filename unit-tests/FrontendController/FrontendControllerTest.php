...
class FrontendControllerTest extends \PHPUnit_Framework_TestCase
{
    public function setUp()
    {
        ...
        $this->routes = $this->getMockBuilder('Symfony\Component\Routing\RouteCollection')->disableOriginalConstructor()->getMock();
        $this->matcher = $this->getMockBuilder('Minima\FrontendController\UrlMatcher')->getMock();
        $this->request = Request::create('path');

        $this->frontendController = new FrontendController($this->routes, $this->matcher, $this->logger);
    }
